...
class VistaHojas(ttk.Frame):

   def __init__(self,*args,**kwargs):
       super().__init__(*args,**kwargs)

       self.Label_title = Label(self,text="Registrar nuevo tipo de hojas ",font=("Arial Black",15)).place(x=300, y=10) 
       ...
       Label(self,text = "Tipo de hojas:",font=("Arial ",13)).place(x=450,y=100)
       ...
       self.tabla.heading('#1',text="NOMBRE hojas ")
       ...
       def BotonMostar():
           ...
           cursor = db_conn.cursor()
           cursor.execute('SELECT * FROM ESPE_HOJAS')
           registros = cursor.fetchall()
           
           for HOJAS in registros:
              self.tabla.insert('',0,text=HOJAS[0],value=(HOJAS[1],))
           #print(registros)
       ...
       def BotonGuardar():
         if (self.entry_suelo == ""): #or self.entry_ID == ""):
           ...
           try:
              cursor = db_conn.cursor()
              hojas=self.VarNombre.get()
              # print(suelo)
              ident=self.VarID.get()
              #print(ident)
          
              query=("INSERT INTO ESPE_HOJAS VALUES ('TYPELEAV{}','{}')".format(ident,hojas))
       ...
       #dentro de boton guardar
            ...
            if (confirm == TRUE):
               ...
               query ="DELETE FROM ESPE_HOJAS WHERE ARL_HOJAS_ID ='%s'"%codigo
