...
class VistaRaiz(ttk.Frame):

   def __init__(self,*args,**kwargs):
       super().__init__(*args,**kwargs)

       self.Label_title = Label(self,text="Registrar nuevo tipo de RAIZ ",font=("Arial Black",15)).place(x=300, y=10) 
       ...
       Label(self,text = "Tipo de raiz :",font=("Arial ",13)).place(x=450,y=100)
       ...
       self.tabla.heading('#1',text="NOMBRE RAIZ ")
      ...
      #BOTONES
       def BotonMostar():
           ...
           cursor = db_conn.cursor()
           cursor.execute('SELECT * FROM ESPE_RAIZ')
           registros = cursor.fetchall()
           
           for RAIZ in registros:
              self.tabla.insert('',0,text=RAIZ[0],value=(RAIZ[1],))
           #print(registros)
       ...
       def BotonGuardar():
         if (self.entry_suelo == ""): #or self.entry_ID == ""):
           ...
           try:
              cursor = db_conn.cursor()
              raiz=self.VarNombre.get()
              # print(suelo)
              ident=self.VarID.get()
              #print(ident)
          
              query=("INSERT INTO ESPE_rAIZ VALUES ('RAIZ{}','{}')".format(ident,raiz))
       ...
       #dentro de boton guardar
            ...
            if (confirm == TRUE):
               ...
               query ="DELETE FROM ESPE_RAIZ WHERE ARL_RAIZ_ID ='%s'"%codigo
       ...
       def BotonBuscar():
            ...
            self.ventana_ed.title("Editar RAIZ")
            self.ventana_ed.geometry("300x200")
            self.ventana_ed.resizable(width=0, height=0)

            self.label_codigo = Label(self.ventana_ed,text="Codigo Raiz:")
       ...
       def OKay():
           ...
           # cursor.execute("SELECT * FROM SUELO WHERE SUELO_ID="+ iden)
            cursor.execute("SELECT * FROM ESPE_RAIZ WHERE RAIZ_ID='{}'" .format(iden))
            registros = cursor.fetchall()
            print(registros)
