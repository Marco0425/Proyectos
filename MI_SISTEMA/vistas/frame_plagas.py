...
class VistaPlagas(ttk.Frame):

   def __init__(self,*args,**kwargs):
       super().__init__(*args,**kwargs)

       self.Label_title = Label(self,text="Registrar nuevo tipo de plagas ",font=("Arial Black",15)).place(x=300, y=10) 
       ...
       Label(self,text = "Tipo de plaga :",font=("Arial ",13)).place(x=450,y=100)
       ...
       self.tabla.heading('#1',text="NOMBRE PLAGAS ")
       ...
       def BotonMostar():
           ...
           cursor = db_conn.cursor()
           cursor.execute('SELECT * FROM CUIDADOS_PLAGAS')
           registros = cursor.fetchall()
           
           for PLAGAS in registros:
              self.tabla.insert('',0,text=PLAGAS[0],value=(PLAGAS[1],))
           #print(registros)
       ...
       def BotonGuardar():
         if (self.entry_suelo == ""): #or self.entry_ID == ""):
           ...
           try: 
              cursor = db_conn.cursor()
              plagas=self.VarNombre.get()
              # print(suelo)
              ident=self.VarID.get()
              #print(ident)
          
              query=("INSERT INTO CUIDADOS_PLAGAS VALUES ('PLAGA{}','{}')".format(ident,plagas))
       ...
       #dentro de boton guardar
            ...
            if (confirm == TRUE):
               ...
               query ="DELETE FROM CUIDADOS_PLAGAS WHERE PLAGAS_ID ='%s'"%codigo
       ...
       def BotonBuscar():
            ...
            self.ventana_ed.title("Editar PLAGA")
            self.ventana_ed.geometry("300x200")
            self.ventana_ed.resizable(width=0, height=0)

            self.label_codigo = Label(self.ventana_ed,text="Codigo PLAGA:")
       ...
       def OKay():
           ...
           # cursor.execute("SELECT * FROM SUELO WHERE SUELO_ID="+ iden)
            cursor.execute("SELECT * FROM CUIDADOS_PLAGAS WHERE PLAGAS_ID='{}'" .format(iden))
            registros = cursor.fetchall()
            print(registros)
